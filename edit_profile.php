...
if(isset($_POST['save'])) {	
	...
	move_uploaded_file($tmp_name, 'uploads/img-clients/' . $img_name);

	if (($_FILES['sp_cart_users_img']['error']) == 4) {
		$newImage = false;
		$updateQuery = "UPDATE clients SET name = '$name', email = '$email' WHERE id = '$id';";
	} else {
		$newImage = true;
		$updateQuery = "UPDATE clients SET name = '$name', email = '$email', image_name = '$img_name' WHERE id = '$id';";	
	}
